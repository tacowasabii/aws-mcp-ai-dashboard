...
import { AWSCredentials, MCPTool, EC2Instance, S3Bucket, AWSAccountInfo } from '../types'
...
export class AWSMCPClient {
  ...
  private async describeEC2Instances(awsConfig: any): Promise<{count: number, instances: EC2Instance[]}> {
    try {
  ...
  private async listS3Buckets(awsConfig: any): Promise<{count: number, buckets: S3Bucket[]}> {
    try {
